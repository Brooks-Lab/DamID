...
for tfn, record in enumerate(record_list):
    ...
    if len(posBsaIF + posBsaIR) == 0:
        ...
        if len(posBsaIF + posBsaIR) <= len(posBsmBIF + posBsmBIR):
            ...
            frame = [round(x / 3 % 1, 1) for x in posBsaIF]
            for x, y in enumerate(posBsaIF):
                # if BsaI is in frame 1, the Gly codon GGT is replaced with GGA
                if frame[x] == 0.3:
                    newtfseq[y + 1] = 'A'
                # if the BsaI site is in frame 2, replace Ser codon TCT with TCA
                elif frame[x] == 0.7:
                    newtfseq[y + 3] = 'A'
                # if the BsaI site is in frame 3, replace Val codon GTC with GTG
                elif frame[x] == 0:
                    newtfseq[y + 2] = 'G'
            # replace Rev BsaI sites (GAGACC) depending on frame
            frame = [round(x / 3 % 1, 1) for x in posBsaIR]
            for x, y in enumerate(posBsaIR):
                # if BsaI is in frame 1, the Glu codon GAG is replaced with GAA
                if frame[x] == 0.3:
                    newtfseq[y + 1] = 'A'
                # if the BsaI site is in frame 2, can't replace Asp because it makes GATC
                ...
                # if N is G, it is Gly and change to GGT
                    ...
                    if newtfseq[y - 2] in ['C', 'A']:
                        ...
                        newtfseq[y] = 'T'
                # if the BsaI site is in frame 3, replace Arg codon AGA with AGG
                elif frame[x] == 0:
                    newtfseq[y + 2] = 'G'
            ...
            frame = [round(x / 3 % 1, 1) for x in posBsmBIF]
            for x, y in enumerate(posBsmBIF):
                # if BsmBI is in frame 1, the Leu codon CTC is replaced with CTT
                if frame[x] == 0.3:
                    newtfseq[y + 4] = 'T'
                # if the BsmBI site is in frame 2, replace Val codon TCT  with TCA
                elif frame[x] == 0.7:
                    newtfseq[y + 3] = 'A'
                # if the BsmBI site is in frame 3, replace Val codon GTC with GTG
                elif frame[x] == 0:
                    newtfseq[y + 2] = 'G'
            # replace Rev BsmBI sites (GAGACG) depending on frame
            frame = [round(x / 3 % 1, 1) for x in posBsmBIR]
            for x, y in enumerate(posBsmBIR):
                # if BsaI is in frame 1, the Thr codon ACG is replaced with ACA
                if frame[x] == 0.3:
                    newtfseq[y + 4] = 'A'
                # if the BsmBI site is in frame 2, replace Asp GAC with GAT
                elif frame[x] == 0.7:
                    newtfseq[y + 3] = 'T'
                # if the BsmBI site is in frame 3, replace Arg codon AGA with AGG
                elif frame[x] == 0:
                    newtfseq[y + 2] = 'G'
            print("  Removed %i BsmBI sites\n  Clone into DamID vector with BsmBI" % len(posBsmBIF + posBsmBIR))
            if [x + 1 for x, y in enumerate(newtfseq) if newtfseq[x:(x + 4)] == ['G', 'A', 'T', 'C']] == [] and \
